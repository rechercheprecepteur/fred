...
// //     // ✅ IMPORTANT: Vider TOUS les caches avant de créer un nouveau token
// //     userCacheMap.clear()  // Vider tout le cache
// //     pendingRequests.clear()  // Vider les requêtes en attente
...
// //         const { data: user, error } = await supabase
// //           .from('users')
// //           .select('id, username, email, role, genre, photo_profil, created_at, updated_at, email_verified')
// //           .eq('id', userId)
// //           .single()
...
//         memoryCache.userId === decoded.userId && 
...
//     const emailHtml = getVerificationEmail(user.username, token, code)
//     await sendEmail(user.email, 'Vérification de votre adresse email', emailHtml)
...
//     if (updateError) return { error: 'Erreur lors du changement de mot de passe' }
...
// export async function getPrecepteurInfo(): Promise<{
//   success?: boolean
//   error?: string
//   precepteurInfo?: any
// }> {
...
//     return { success: true, precepteurInfo: data }

//   } catch (error) {
//     console.error('getPrecepteurInfo error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//   } catch (error) {
//     console.error('updatePrecepteurDisponibility error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//   } catch (error) {
//     console.error('getParentProfile error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//   } catch (error) {
//     console.error('updateParentProfile error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//     const { data: users, error, count } = await supabase
//       .from('users')
//       .select('id, username, email, role, genre, photo_profil, email_verified, created_at, updated_at', { count: 'exact' })
//       .order('created_at', { ascending: false })
...
//   } catch (error) {
//     console.error('verifyPrecepteur error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//   } catch (error) {
//     console.error('getPrecepteursEnAttente error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
//   } catch (error) {
//     console.error('getAllPrecepteurs error:', error)
//     return { error: 'Erreur serveur' }
//   }
// }
...
// actions/auth.ts - Version optimisée de getAllUsers
export async function getAllUsers(): Promise<{
  success?: boolean
  error?: string
  users?: any[]
  ...
  try {
    ...
    const formattedUsers = users?.map(user => ({
      ...user,
      precepteurDetails: user.precepteurs?.[0] || null,
      precepteurs: undefined // Nettoyer la propriété
    })) || []

    return { success: true, users: formattedUsers, total: count || 0 }
