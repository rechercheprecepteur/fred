...
// components/SessionGrades.tsx
'use client'

import { useState, useEffect } from 'react'
import { getSessionGrades, createSessionGrade, updateSessionGrade, deleteSessionGrade } from '@/lib/session-api'
import { Plus, Trash2, Award, Star, Edit3, X, Check } from 'lucide-react'
...
export default function SessionGrades({ sessionId }: SessionGradesProps) {
  ...
  const loadGrades = async () => {
    try {
      const data = await getSessionGrades(sessionId)
      setGrades(data.grades || [])
    } catch (error) {
      console.error('Erreur chargement cotations:', error)
    }
    setLoading(false)
  }

  const resetForm = () => {
    setForm({ titre: '', score: '', max_score: '20', comment: '' })
    setEditingId(null)
    setShowForm(false)
  }

  // ✅ Soumettre via API Express
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    ...
    try {
      if (editingId) {
        await updateSessionGrade(editingId, gradeData)
      } else {
        await createSessionGrade(sessionId, gradeData)
      }
      await loadGrades()
      resetForm()
    } catch (error) {
      console.error('Erreur sauvegarde cotation:', error)
    }
  }
  ...
  // ✅ Supprimer via API Express
  const handleDelete = async (gradeId: number) => {
    try {
      await deleteSessionGrade(gradeId)
      await loadGrades()
    } catch (error) {
          ...
          {grades.map((grade) => {
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-1.5">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-gray-700 truncate">{grade.titre}</p>
                    {grade.comment && (
                      <p className="text-xs text-gray-400 mt-0.5">{grade.comment}</p>
                    )}
                  </div>
