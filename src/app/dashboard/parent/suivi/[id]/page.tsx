...
export default function SuiviEnfantPage() {
  ...
  // ✅ Charger les données via API Express
  const loadSuiviData = async () => {
    setLoading(true)
    setError('')
    ...
    try {
      ...
      // 1. Charger l'élève
      const eleveResponse = await fetch(`${API_URL}/eleves/${enfantId}`, {
        headers: {
      ...
      if (eleveData.success) {
        setEleve(eleveData.eleve)
      }

      // 2. Charger les contrats du parent
      const contratsResponse = await fetch(`${API_URL}/auth/contracts`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
      ...
      if (contratsData.success && contratsData.contrats) {
        // Filtrer les contrats pour cet élève
        const contratsEnfant = contratsData.contrats.filter((c: any) => 
          String(c.eleve_id) === String(enfantId) ||
          String(c.eleve?.id) === String(enfantId)
        )

        if (contratsEnfant.length > 0) {
          const contratsEnrichis: ContratWithDetails[] = await Promise.all(
            contratsEnfant.map(async (contrat: any) => {
              // Récupérer les sessions pour ce contrat
              let sessions: SessionWithDetails[] = []
              try {
                const sessionsResponse = await fetch(
                  `${API_URL}/auth/precepteur/contrats/${contrat.id}/sessions`,
                  {
                    headers: {
                      'Authorization': `Bearer ${token}`,
                      'Content-Type': 'application/json'
                ...
                sessions = contrat.sessions || []
              }

              return {
                id: contrat.id,
                matiere_nom: contrat.matiere?.nom || contrat.matiere || 'Matière',
                matiere_niveau: contrat.matiere?.niveau || contrat.niveau || '',
                precepteur_username: contrat.precepteur?.user?.username || 'Précepteur',
                precepteur_photo: contrat.precepteur?.user?.photo_profil || null,
                precepteur_email: contrat.precepteur?.user?.email || null,
                precepteur_telephone: contrat.precepteur?.user?.telephone || contrat.precepteur?.telephone || null,
                precepteur_note: contrat.precepteur?.note_moyenne || 0,
                precepteur_commune: contrat.precepteur?.commune || '',
                precepteur_quartier: contrat.precepteur?.quartier || '',
                date_debut: contrat.date_debut || '',
                date_fin: contrat.date_fin || '',
                heure_debut_pref: contrat.heure_debut_pref || '',
                heure_fin_pref: contrat.heure_fin_pref || '',
                jours_pref: contrat.jours_pref || '',
                tarif_horaire: contrat.tarif_horaire || contrat.tarif_final || null,
                statut: contrat.statut || 'en_attente',
                sessions: sessions
              }
            })
          )
          ...
          setContrats(contratsEnrichis)
          if (contratsEnrichis.length > 0) {
            setSelectedContrat(String(contratsEnrichis[0].id))
          }
        }
      }
    } catch (err) {
  ...
  const selectedContratData = contrats.find(c => String(c.id) === String(selectedContrat))
          ...
          <div className="flex flex-wrap gap-2">
            {contrats.map((contrat) => (
              <button
                ...
                onClick={() => setSelectedContrat(String(contrat.id))}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${
                  String(selectedContrat) === String(contrat.id)
                    ? 'bg-black text-white shadow-lg'
                    : 'bg-white text-gray-600 hover:bg-gray-50 border border-gray-200'
                }`}
              >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  {selectedContratData.precepteur_photo ? (
                    <img src={getPhotoUrl(selectedContratData.precepteur_photo) || ''} alt="" className="w-6 h-6 rounded-full object-cover" />
                  ) : (
                    <User className="w-5 h-5 text-gray-400" />
                  )}
                  <span className="font-medium text-gray-900">{selectedContratData.precepteur_username}</span>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {selectedContratData.sessions.length === 0 ? (
              ...
              <div className="divide-y divide-gray-100">
                {selectedContratData.sessions.map((session) => (
                  <div key={String(session.id)} className="p-4 hover:bg-gray-50/50">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getSessionStatutColor(session.statut)}`}>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="space-y-1.5">
                          {session.grades.map((grade) => {
                            const percentage = (grade.score / grade.max_score) * 100
                            return (
                              <div key={String(grade.id)} className="flex items-center gap-3 bg-gray-50 p-2 rounded-lg">
                                <div className="flex-1 min-w-0">
                                  <div className="flex items-center justify-between mb-1">
