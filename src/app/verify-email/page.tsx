...
export default function VerifyEmailPage() {
  ...
  const [status, setStatus] = useState<'code_input' | 'success' | 'error'>('code_input')
  const [message, setMessage] = useState('')
  const [messageType, setMessageType] = useState<'success' | 'error'>('success')
  const [code, setCode] = useState(['', '', '', '', '', ''])
  const [email, setEmail] = useState(emailParam || '')
  ...
  const handleResendCode = async () => {
    ...
    try {
      // ✅ Appel à notre API Express
      const result = await resendVerificationEmail(email)
      console.log('📥 Résultat renvoi code:', result)
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="text-center">
                <button
                  ...
                  className="text-sm text-indigo-600 hover:text-indigo-800 hover:underline font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                >
