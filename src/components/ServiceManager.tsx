...
// import { getServices, createService, updateService, deleteService, toggleServiceActive } from '@/actions/services'
...
export default function ServiceManager({ precepteurId, isOwner = false }: ServiceManagerProps) {
  ...
  // ✅ CHANGEMENT 1 : loadServices avec Express
  const loadServices = useCallback(async () => {
    if (!precepteurId) {
      setLoading(false)
      return
    ...
    const { services: data, error: loadError } = await getServices()
  ...
  // ✅ CHANGEMENT 2 : handleCreate avec Express
  const handleCreate = async (formData: any) => {
    if (!precepteurId) return
    setSaving(true)
  ...
  // ✅ CHANGEMENT 3 : handleUpdate avec Express
  const handleUpdate = async (formData: any) => {
    if (!precepteurId || !editingService) return
    setSaving(true)
  ...
  // ✅ CHANGEMENT 4 : handleDelete avec Express
  const handleDelete = async () => {
    if (!precepteurId || !deletingService) return
    setDeleting(true)
  ...
  // ✅ CHANGEMENT 5 : handleToggleActive avec Express
  const handleToggleActive = async (service: ServicePrecepteur) => {
